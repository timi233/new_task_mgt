<template>
  <div v-if="isDesktop" class="desktop-page user-management-desktop">
    <el-card shadow="never" class="filter-card">
      <el-form inline>
        <el-form-item>
          <el-input
            v-model="keyword"
            placeholder="搜索姓名/电话/邮箱/部门"
            clearable
            @keyup.enter="handleSearch"
          />
        </el-form-item>
        <el-form-item>
          <el-select v-model="filters.functionalRole" placeholder="功能权限" clearable>
            <el-option
              v-for="opt in functionalRoleOptions"
              :key="opt.value ?? 'null'"
              :label="opt.text"
              :value="opt.value"
            />
        ...
        <el-form-item>
          <el-select v-model="filters.responsibilityRole" placeholder="职责权限" clearable>
            <el-option
              v-for="opt in responsibilityRoleOptions"
              :key="opt.value ?? 'null'"
              :label="opt.text"
              :value="opt.value"
            />
        ...
        <el-form-item>
          <el-select v-model="filters.status" placeholder="状态" clearable>
            <el-option v-for="opt in statusOptions" :key="opt.value" :label="opt.text" :value="opt.value" />
          </el-select>
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item>
          <el-button type="success" :loading="syncing" @click="handleSyncFeishu">同步飞书</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    ...
    <el-card shadow="never">
      <el-table :data="users" v-loading="loading">
          ...
          <template #default="{ row }">
            <div class="user-cell">
              ...
              <div>
                ...
                <div class="dept">{{ row.department || '无部门' }}</div>
              </div>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="220">
          <template #default="{ row }">
            <el-button text size="small" @click="showPermissionPopup(row)">修改权限</el-button>
            <el-button
              size="small"
    ...
    >
      <div class="permission-section" v-if="isAdmin(currentUserData)">
        ...
        <el-radio-group v-model="selectedFunctionalRole">
          <el-radio :label="null">无</el-radio>
          <el-radio :label="FunctionalRole.TECHNICIAN">技术员</el-radio>
          <el-radio :label="FunctionalRole.SALES">销售</el-radio>
        </el-radio-group>
      </div>
      <div class="permission-section" v-if="isSystemAdmin(currentUserData)">
        <div class="section-title">职责权限</div>
        <el-radio-group v-model="selectedResponsibilityRole">
          <el-radio :label="ResponsibilityRole.SYSTEM_ADMIN">系统管理员</el-radio>
          <el-radio :label="ResponsibilityRole.ADMIN">管理员</el-radio>
          <el-radio :label="ResponsibilityRole.AUDITOR">审计</el-radio>
          <el-radio :label="ResponsibilityRole.OTHER">其他</el-radio>
        </el-radio-group>
      </div>
      <template #footer>
        <el-button @click="showPermissionPicker = false">取消</el-button>
        <el-button type="primary" @click="handleUpdatePermission">确认</el-button>
      </template>
    </el-dialog>
  </div>

  <div v-else class="user-management-page">
