...
import { prisma, success, paginate, generateOrderNo, createModuleLogger, LogModule } from '../utils';
import { MessageService } from '../feishu/messageService';
import { ApprovalService, ApprovalStatus } from '../feishu/approvalService';
import { Role, OrderType, OrderStatus, Priority, isCompanyOrder, isManufacturerOrder, isFieldOrder, isOfficeOrder, hasNoPermission, hasManagementRole, isTechnician, isSales } from '../types';
...
router.get('/', authorize(...canViewOrders), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (technicianId) {
      andConditions.push({
        technicians: {
          some: { technicianId: technicianId as string },
        },
      });
    }
...
router.post('/', authorize(...canManageOrders), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
      ...
      try {
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
          ...
          if (attempt === ORDER_NO_RETRY_LIMIT - 1) {
            throw ApiError.internal('生成工单编号失败，请稍后重试');
          }
          continue;
        }
        throw error;
    ...
    if (!order) {
      throw ApiError.internal('创建工单失败');
    }
...
router.post('/:id/accept', authorize(...canManageOrders), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (isFieldOrder(order.orderType)) {
...
function buildDateTime(dateStr: string, period: string | undefined, type: 'start' | 'end'): Date {
...
function maskName(value?: string | null): string | undefined | null {
  if (!value) return value;
  if (value.length <= 1) return `${value}*`;
  return `${value[0]}${'*'.repeat(Math.min(value.length - 1, 3))}`;
}

function maskPhone(value?: string | null): string | undefined | null {
  if (!value) return value;
