...
export class FeishuService {
  ...
  async getTenantAccessToken(): Promise<string> {
    ...
    try {
      ...
      console.error('获取飞书tenant_access_token失败:', error);
      throw error;
    }
  }
  ...
  async getUserByCode(code: string): Promise<FeishuUserInfo> {
    try {
      ...
      console.log('[飞书] 获取到 tenant_access_token');
      ...
      console.log('[飞书] access_token 响应 code:', tokenResponse.data.code);
      ...
      console.log('[飞书] user_info 响应:', JSON.stringify(userInfoResponse.data, null, 2));
      ...
      if (!userData || !userData.open_id) {
        console.error('[飞书] 用户数据异常:', userData);
      ...
      console.error('[飞书] 获取用户信息失败:', error.response?.data || error.message);
      throw error;
    }
  }
  ...
  async getUserIdByMobile(mobile: string): Promise<string | null> {
    try {
      ...
      if (response.data.code !== 0) {
        console.error('查询用户ID失败:', response.data.msg);
        return null;
      }
      ...
      console.error('获取飞书用户ID失败:', error);
      return null;
    }
  }
  ...
  async getDepartments(parentId: string = '0'): Promise<any[]> {
    try {
      ...
      do {
        ...
        if (response.data.code !== 0) {
          console.error('获取部门列表失败:', response.data.msg, response.data.code);
          break;
        }

        const items = response.data.data?.items || [];
        console.log(`[飞书] 获取到 ${items.length} 个部门`);
      ...
      console.error('获取飞书部门列表失败:', error.response?.data || error.message);
      return [];
    }
  }
  ...
  async getDepartmentMembers(departmentId: string): Promise<any[]> {
    ...
    if (departmentId === '0') {
      console.log('[飞书] 跳过根部门（ID=0）成员获取');
      return [];
    }
    ...
    try {
      ...
      do {
        ...
        if (response.data.code !== 0) {
          console.error(`[飞书] 获取部门 ${departmentId} 成员失败:`, response.data.msg, response.data.code);
          break;
        }

        const items = response.data.data?.items || [];
        console.log(`[飞书] 部门 ${departmentId} 获取到 ${items.length} 个成员`);
        members.push(...items);
        pageToken = response.data.data?.page_token;
      } while (pageToken);
      ...
      console.error(`[飞书] 获取部门 ${departmentId} 成员失败:`, error.response?.data || error.message);
      return [];
    }
  }
  ...
  async getUserInfo(userId: string): Promise<any | null> {
    try {
      ...
      if (response.data.code !== 0) {
        console.error('获取用户详情失败:', response.data.msg);
        return null;
      }

      return response.data.data?.user;
    } catch (error) {
      console.error('获取飞书用户详情失败:', error);
      return null;
    }
  }
}
